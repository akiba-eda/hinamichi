...
export default route({ methods: ["POST"], auth: "user" }, async (req, _res, ctx) => {
  ...
  const b = z.object({ shelterId: z.string(), full: z.boolean().default(true), lat: z.number(), lng: z.number(), locationSource: z.enum(["gps", "cached", "manual"]).default("gps") }).parse(body(req));
